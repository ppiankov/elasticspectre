...
Elasticsearch cluster security scanner.
...
```
go install github.com/ppiankov/elasticspectre/cmd/elasticspectre@latest
```
...
Scans Elasticsearch clusters for security findings.
...
{
  "version": "spectre/v1",
  "scanner": "elasticspectre",
  "target": "Elasticsearch clusters",
  "findings": [
    {
      "id": "FIND-001",
      "severity": "high",
...
- Does not remediate or modify Elasticsearch clusters — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated Elasticsearch clusters monitoring — point-in-time security audit only
...
```bash
elasticspectre scan --output json | jq '.summary'
elasticspectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
